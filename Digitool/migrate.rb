...
begin 
	...
	# for each record
	document.xpath('/oai:OAI-PMH/oai:ListRecords/oai:record', oai_ns)[0,1].each do |r| 
		...
		r.xpath('oai:metadata/marc:record/marc:datafield[@tag="856"]/marc:subfield[@code="u"]', oai_ns).each_with_index do |f,i|
			...
			log "Uploading to S3"
			write_file(s3_bucket, "#{@inst}/upload/#{import_profile_id}/#{ingest_id}/#{filename}", "#{temp_folder}#{identifier}/#{filename}")
	...
	# TODO: Write marcxml file to ingest location
